...
export function Blocks({ blocks, cardId }) {
  ...
  return blocks.map((b, i) => {
    ...
    if (b.type === "journey-map")
      ...
      // adjustment). Width fills from that text position to the right viewport
      // edge: 100vw minus sidebar (ml-60 = 15rem) minus left content padding
      // (px-10 = 40px). If either value changes, update the calc() here.
      return (
        <div key={i} style={{ width: "calc(100vw - 15rem - 40px)" }}>
          <ClientJourneyMap />
        </div>
      );
